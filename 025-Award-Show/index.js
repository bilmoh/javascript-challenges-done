...
function assignAwards(data) {
    // use getHost() to get a flat array of podcast hosts
    const hosts = getHosts(data)

    // map through my array of hosts for each:
    return hosts.map(host => {
        // use Math.random to generate a random number between 0 and length of award arr
        const randIndex = Math.floor(Math.random() * awards.length)

        return `${awards[randIndex]} ${hosts}`
    })
    // use the random number to access a random award index
